...
void PeriodLight2(){
  if (millis() - period_milis_light2_prev > period_milis_light2)  
   { 
      ...
      int period = millis() - period_milis_light1_prev - period_milis_light2;           
...
int period_milis_pump1 = 48*60*60*1000;
int flow_pump1 = 30*1000;
...
int period_milis_pump2 = 48*60*60*1000;
int flow_pump2 = 15*1000;
int period_milis_pump2_prev = 0;
void PeriodPump2(){
  if (millis() - period_milis_pump1_prev > period_milis_pump2)  
   { 
...
void Status(){
  ...
  int __millis = millis();
  String status_ =  "Свет № 1 - " +   GetStatusDevice(controlLight1) 
  + Strgin(" Таймер включение ") + GetStatusDevice((period_milis_light1 - __millis + period_milis_light1_prev))/60*1000) + Strgin(" Таймер выключение ") 
  + GetStatusDevice((flow_light2 - (__millis - period_milis_light1_prev - period_milis_light1))/60*1000) + String("\n")
  
  +String("Свет № 2 - "         ) +   GetStatusDevice(controlLight2) + Strgin"" 
    + Strgin(" Таймер включение ") + GetStatusDevice((period_milis_light2 - __millis + period_milis_light2_prev)/60*1000) + Strgin(" Таймер выключение ") 
  + GetStatusDevice((flow_light2 - (__millis - period_milis_light2_prev - period_milis_light2))/60*1000) + String("\n")
  
  +String("Насос № 1 - "      ) +     GetStatusDevice(controlPump1)  + Strgin""  
    + Strgin(" Таймер включение ") + GetStatusDevice((period_milis_pump1 - __millis + period_milis_pump1_prev)/60*1000) + Strgin(" Таймер выключение ") 
  + GetStatusDevice((flow_pump1 - (__millis - period_milis_pump1_prev - period_milis_pump1))/60*1000) + String("\n")
  
  +String("Насос № 2 - "      ) +     GetStatusDevice(controlPump2)  +
    + Strgin(" Таймер включение ") + GetStatusDevice((period_milis_pump2 - __millis + period_milis_pump2_prev)/60*1000) + Strgin(" Таймер выключение ") 
  + GetStatusDevice((flow_pump2 - (__millis - period_milis_pump2_prev - period_milis_pump2))/60*1000) + String("\n")
