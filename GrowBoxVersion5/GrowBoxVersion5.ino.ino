...
void Status(){
  ...
  {
    remain_time1 = 24*60*60*100 - remain_time1;
  }
  
  int remain_time2 = __millis - period_milis_light2_prev;
  if(remain_time2 > period_milis_light2)
  {
    remain_time2 = 24*60*60*100 - remain_time2;
  }
  
  String status_1 =  String("№1") + GetStatusDevice(controlLight1) + String(remain_time1);
    
   String status_2 =  String("№2") + GetStatusDevice(controlLight2) + String(remain_time2);
 ...
 String pump_2 = String("Н_№2") + GetStatusDevice(controlPump1) 
  ...
  myKbd.addButton("Текущий статус" + status_1 + status_2 + pump_1 + pump_2 , STATUS_CALLBACK, CTBotKeyboardButtonQuery);
  ...
  myKbd.addButton("Цвет", VEGA_CALLBACK, CTBotKeyboardButtonQuery);
  myKbd.addButton("Вега", BLOOM_CALLBACK, CTBotKeyboardButtonQuery);
  // 3
  myKbd.addRow();
  ...
  myBot.sendMessage(chat_id, "Гроубокс", myKbd);
}
